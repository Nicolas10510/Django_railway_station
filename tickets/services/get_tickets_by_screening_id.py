...
class TicketsCollector:
    ...
    def tickets(self) -> dict[int, list[Place]]:
        for row in self.__tickets:
            ...
            self.__tickets[row] = [
                Place(is_taken=True if place in places else False, row=row, place=place) for place in range(1, 5)
            ]
...
def get_tickets_by_screening_id(screening_id: int) -> tuple[dict[int, list[Place]], list[Ticket]]:
    return TicketsCollector(screening_id).tickets, Screening.objects.select_related('poezd_id').get(id=screening_id)
